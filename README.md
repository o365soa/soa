...
* Azure AD applications are scoped only to certain activities, which are documented above. Scopes are used only when they are required, and the assessment follows a least-privilege model. Most scopes are read-only and specific to configuration settings, not access to any user content.
